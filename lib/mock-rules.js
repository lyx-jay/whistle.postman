...
MockRulesManager.prototype.createMock = function(options) {
  ...
  // Deduplication: check if mock with same urlPath and method exists
  var mocks = this.getMocks();
  var existingIndex = -1;
  for (var i = 0; i < mocks.length; i++) {
    if (mocks[i].urlPath === urlPath && 
        (mocks[i].method || 'GET').toUpperCase() === method &&
        mocks[i].ruleType === ruleType) {
      existingIndex = i;
      break;
    }
  }
  ...
  if (existingIndex >= 0) {
    var existing = mocks[existingIndex];
    existing.name = options.name || existing.name;
    existing.responseBody = options.responseBody || existing.responseBody;
    existing.ruleContent = options.ruleContent || existing.ruleContent;
    existing.statusCode = options.statusCode || existing.statusCode;
    existing.headers = options.headers || existing.headers;
    existing.enabled = true;
    existing.updatedAt = new Date().toISOString();
    
    if (ruleType === 'mock') {
      this.values.set(existing.id, existing.responseBody);
    }
    
    this._saveMocks(mocks);
    return { mock: existing, updated: true };
  }
  ...
  var mock = {
    id: id,
    name: options.name || (ruleType.charAt(0).toUpperCase() + ruleType.slice(1)) + ' ' + method + ' ' + urlPath,
    url: options.url,
    urlPath: urlPath,
    method: method,
    ruleType: ruleType,
  ...
  return { mock: mock, updated: false };
};
